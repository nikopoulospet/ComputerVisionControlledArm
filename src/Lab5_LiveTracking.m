...
while size(objects) ~= 0
    ...
    for ball=1:1:1
        ...
        pos = pp.getPosition();
        startingPosition = transpose(pp.fk3001(pos));
        ...
        points = [startingPosition; topPosition; grabPosition; topPosition; raisePosition; sortPosition; homePosition];
        ...
        while k <= 7
            ...
            while 1
                ...
                %   if(isequal(finalPosition, topPosition))
                [objects,colors] = CAM.detectImproved();
                for i = 1:1:size(colors)
                    if(colors(i) == target_color)
                        Object_location = [objects(i, 1) objects(i,2)];
                    end
                end
                currentPos = pp.getPosition();
                initialPosition = pp.fk3001(currentPos);
                finalPosition = [Object_location(1)-20, Object_location(2), 70];
                %        topPosition = finalPosition;
                %      disp("TRACKING");
                %     end
                deltaTime = toc;
