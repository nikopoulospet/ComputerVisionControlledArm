...
for i = 1:1:5
    ...
    subplot(1, 2, 1)
    joint_angles = transpose(pp.ik_3001_numerical(target_pos,model));
    ...
    packet = pp.setSetpoints_Improved(joint_angles, 2500);
    ...
    while(toc < 3)
        ...
        hold on;
        % plot vector between end effector position and target
        quiver3(endEffector_pos(1),endEffector_pos(2),endEffector_pos(3),target_pos(1)-endEffector_pos(1),target_pos(2)-endEffector_pos(2),target_pos(3)-endEffector_pos(3));
        hold off;
        view(0,0);
        pause(0.1);
    end
